...
class VimeoDataset(Dataset):
    ...
    def load_data_new(self): 
        ...
        test_fn = os.path.join(self.data_root, 'vallist.txt')
        flow_fn = os.path.join(self.data_root, 'flowlist.txt')
        ...
        with open(flow_fn, 'r') as f:
            self.flow_data = f.read().splitlines()
    ...
    def load_data(self):
        ...
        for i in tqdm(range(self.num)):
            with np.load('train_npz/{}.npz'.format(i)) as f:
                self.train_data.append(f['i0i1gt'])
                self.flow_data.append(f['ft0ft1'])

        for i in tqdm(range(self.num2)):
            with np.load('val_npz/{}.npz'.format(i)) as f:
            # with np.load('dataset_val/{}.npz'.format(i)) as f:
            # f = np.load('dataset_val/{}.npz'.format(i)) #,mmap_mode='r'
    ...
    def getimg(self, index):
        ...
        flow_data=np.load(self.flow_data[index])
